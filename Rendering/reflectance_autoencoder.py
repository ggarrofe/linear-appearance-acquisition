...
if __name__ == "__main__":
    ...
    autoencoder = net.LinearAutoEncoder(pos_size=input_ch, latent_size=args.latent_size, num_clusters=args.num_clusters).to(device)
    optimizer = torch.optim.Adam([
        {
            "params": autoencoder.encoder.parameters(), 
            "lr": args.lrate
        }
    ])
    ...
    if wandb.run.resumed:
        ...
        autoencoder.load_state_dict(checkpoint['encoder_state_dict'])
    ...
    while epoch < args.num_epochs:
        ...
        embed_X = embed_fn(X_NdotL_NdotH[indices].to(device))
        input = torch.cat([embed_X, autoencoder.encode(embed_X)], dim=-1)
        ...
        pred_rgb = autoencoder(input.to(device), 
                            cluster_ids[indices], 
                            linear_mappings)
        ...
        embed_X = embed_fn(X_NdotL_NdotH_val[indices].to(device))
        input = torch.cat([embed_X, autoencoder.encode(embed_X)], dim=-1)
        pred_rgb = autoencoder(input.to(device), cluster_ids_val, linear_mappings)
        ...
        if epoch%10 == 0:
            ...
            embed_X = embed_fn(X_NdotL_NdotH_i.to(device))
            input = torch.cat([embed_X, autoencoder.encode(embed_X)], dim=-1)
            pred_rgb = autoencoder(input, cluster_ids_i)
            
            v.validation_view_reflectance(reflectance=pred_rgb.detach().cpu(),
                                        target=img.detach().cpu(),
                                        linear=autoencoder.position_mapping(embed_fn(X_NdotL_NdotH_i), cluster_ids_i).detach().cpu(),
            ...
            torch.save({ # Save our checkpoint loc
                'epoch': epoch,
                'encoder_state_dict': autoencoder.state_dict(),
