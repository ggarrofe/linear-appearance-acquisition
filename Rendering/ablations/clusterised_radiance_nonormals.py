...
if __name__ == "__main__":
    ...
    dataset.switch_2_xv_dataset(device if args.dataset_to_gpu else torch.device("cpu"))
    ...
    if not args.only_eval:
        xv, target_rgb, depths = dataset.get_tensors("train", device=device)

        linear_mappings = torch.zeros([args.num_clusters, 3, input_ch]).to(device)
        cluster_ids = torch.zeros((xv.shape[0],), dtype=torch.long).to(device)
        centroids = torch.zeros((args.num_clusters, 3)).to(device)

        mask = (xv[:, 0] == -1.) & (xv[:, 1] == -1.) & (xv[:, 2] == -1.)  # not masking takes too much time

        start_time = time.time()
        cluster_ids[~mask], centroids[:args.num_clusters-1] = kmeans(X=xv[~mask, :3],
        ...
        centroids[args.num_clusters-1] = torch.tensor([-1., -1., -1.]).to(xv)
        ...
        for cluster_id in tqdm(range(args.num_clusters-1), unit="linear mapping", desc="Computing linear mappings"):
            linear_mappings[cluster_id] = compute_inv(xv, target_rgb, cluster_id, cluster_ids, embed_fn=embed_fn, device=device)
        ...
        xv_tr, img_tr, depths_tr = dataset.get_X_target("train", 0, device=device)
        cluster_ids_tr = kmeans_predict(xv_tr[..., :3], centroids, device=device)
        v.plot_clusters_3Dpoints(xv_tr[..., :3], cluster_ids_tr, args.num_clusters,
    ...
    start_time = time.time()
    cluster_ids_tr = kmeans_predict(xv_tr[..., :3], centroids, device=device)
    pred_rgb_tr = linear_net(xv_tr, cluster_ids_tr)
