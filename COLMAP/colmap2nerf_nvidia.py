...
def parse_args():
    # python3 colmap2nerf.py --images ./lego/images --out ./lego/nerf_transforms/transforms.json --text ./lego/sparse --colmap_db ./lego/database.db
    # python3 colmap2nerf.py --images ./lego_nvidia/images --run_colmap --colmap_matcher exhaustive --out ./lego_nvidia/transforms.json
...
def run_colmap(args):
    ...
    if os.path.exists(db):
        os.remove(db)
    do_system(f"/data/gg921/bin/colmap feature_extractor --ImageReader.camera_model OPENCV --ImageReader.single_camera 1 --database_path {db} --image_path {images}")
    do_system(f"/data/gg921/bin/colmap {args.colmap_matcher}_matcher --database_path {db}")
    try:
        shutil.rmtree(sparse)
    except:
        pass
    do_system(f"mkdir {sparse}")
    do_system(f"/data/gg921/bin/colmap mapper --database_path {db} --image_path {images} --output_path {sparse}")
    do_system(f"/data/gg921/bin/colmap bundle_adjuster --input_path {sparse}/0 --output_path {sparse}/0 --BundleAdjustment.refine_principal_point 1")
    try:
        shutil.rmtree(text)
    except:
        pass
    do_system(f"mkdir {text}")
    do_system(f"/data/gg921/bin/colmap model_converter --input_path {sparse}/0 --output_path {text} --output_type TXT")
